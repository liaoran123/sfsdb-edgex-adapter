...
func TestMQTTIntegration(t *testing.T) {
	...
	// 初始化数据队列
	dataQueue, err := queue.NewQueue("./test_data_queue")
	if err != nil {
		t.Fatalf("Failed to initialize data queue: %v", err)
	}
	defer os.RemoveAll("./test_data_queue")
	...
	// 发布消息到 MQTT 主题
	if err := mqttClient.Publish("edgex/events/core/test", 1, false, testMessage); err != nil {
